...
export const handlers = [
  ...
  http.post(`${TAURI_ENDPOINT}/get_installed_skills`, () =>
    success(getInstalledSkills()),
  ),

  http.post(`${TAURI_ENDPOINT}/migrate_skill_storage`, () =>
    success({
      migratedCount: getInstalledSkills().length,
      skippedCount: 0,
      errors: [],
    }),
  ),
  ...
  http.post(`${TAURI_ENDPOINT}/list_vscode_plugin_targets`, () =>
    success([
      {
        id: "claude",
        label: "Claude Code",
      ...
      },
      {
        ...
        configPaths: [
          ...
          "/mock/home/.codex/auth.json",
        ],
        status: "detected",
        message: "Codex VS Code 插件复用 ~/.codex 配置与认证文件",
      },
      {
        id: "kilo",
        label: "Kilo",
        ...
        installed: false,
        version: null,
        configPaths: ["/mock/home/.config/kilo/kilo.jsonc"],
      ...
      },
      {
        ...
        version: null,
        configPaths: [
          "/mock/home/.config/opencode/opencode.json",
          "/mock/home/.local/share/opencode/auth.json",
        ],
        status: "not_installed",
        message: null,
      },
  ...
  http.post(
    `${TAURI_ENDPOINT}/read_vscode_plugin_status`,
    async ({ request }) => {
      const { targetId } = await withJson<{ targetId: string }>(request);
      return success({
        id: targetId,
        label: targetId,
  ...
  http.post(`${TAURI_ENDPOINT}/sync_vscode_plugin_provider`, () =>
    success(true),
  ),

  http.post(`${TAURI_ENDPOINT}/clear_vscode_plugin_provider`, () =>
    success(true),
  ),

  http.post(
    `${TAURI_ENDPOINT}/preview_vscode_plugin_changes`,
    async ({ request }) => {
      const { targetId } = await withJson<{ targetId: string }>(request);
      return success({
        targetId,
