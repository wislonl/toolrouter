...
describe("SettingsPage integration", () => {
  it("loads default settings from MSW", async () => {
    ...
    expect((appInput as HTMLInputElement).value).toBe("/home/mock/.ak-switch");
  });

  it("uses installed skills from MSW before confirming skill storage migration", async () => {
    renderDialog();
    ...
    fireEvent.click(screen.getByText("settings.skillStorage.unified"));

    await screen.findByText("settings.skillStorage.confirmTitle");
    expect(
      screen.getByText("settings.skillStorage.confirmMessage"),
    ).toBeInTheDocument();
  });
  ...
  it("allows browsing and resetting directories", async () => {
    ...
    expect(appInput.value).toBe("/home/mock/.ak-switch");

    fireEvent.click(browseButtons[0]);
    await waitFor(() =>
      expect(appInput.value).toBe("/home/mock/.ak-switch/picked"),
    );

    fireEvent.click(resetButtons[0]);
    await waitFor(() => expect(appInput.value).toBe("/home/mock/.ak-switch"));
