...
> 下方截图只使用演示数据。流量和 Token 统计为展示用数据，不展示真实 API Key。
...
|                        用量与 Token 统计                         |
| :--------------------------------------------------------------: |
| ![ToolRouter 用量与 Token 统计](assets/screenshots/usage-zh.png) |
...
- **6 个受管工具，50+ 预设** — Claude Code、Codex、Gemini CLI、OpenCode、OpenClaw、Hermes；复制 key 即可一键导入
- **通用供应商** — 一份配置同步到多个应用（OpenCode、OpenClaw、Hermes）
- **VS Code 插件** — VS Code 的 Claude、Codex、Kilo 和 OpenCode 插件集成
...
- **Deep Link** (`akswitch://`) — 通过 URL 一键导入供应商、MCP 服务器、提示词和技能
...
- **数据库**：`~/.ak-switch/ak-switch.db`（SQLite — 供应商、MCP、提示词、技能）
- **本地设置**：`~/.ak-switch/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.ak-switch/backups/`（自动轮换，保留最近 10 个）
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.ak-switch/ak-switch.db`（SQLite）
