...
fn import_default_config_claude_persists_provider() {
    ...
    let db_path = home.join(".ak-switch").join("ak-switch.db");
    assert!(
        db_path.exists(),
        "importing default config should persist to ak-switch.db"
    );
}
...
fn import_mcp_from_claude_creates_config_and_enables_servers() {
    ...
    assert!(
        db_path.exists(),
        "state.save should persist to ak-switch.db when changes detected"
    );
}
