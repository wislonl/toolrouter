...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- ToolRouter SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 ToolRouter 导出的 SQL 备份文件。",
            "Only SQL backups exported by ToolRouter are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("ak-switch.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("ak-switch-periodic-maintenance-backup-disabled-test");
