...
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "ak-switch-model-catalog.json";
...
/// ak-switch–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// ak-switch–generated file (identified by filename
/// `ak-switch-model-catalog.json`). A user-managed external catalog file is
...
/// Given `config.toml` text, resolve the on-disk path of the ak-switch–owned
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/ak-switch-model-catalog.json");
        ...
        assert_eq!(
            parsed
                .get("model_catalog_json")
                .and_then(|value| value.as_str()),
            Some("/tmp/ak-switch-model-catalog.json")
        );
        assert!(
            parsed
                .get("model_providers")
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/ak-switch-model-catalog.json");
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/ak-switch-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/ak-switch-model-catalog.json"
"#;
