...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a ak-switch private field that lives only in
            // the DB SSOT plus the `ak-switch-model-catalog.json` projection
...
/// into the ToolRouter database. Each provider found will be added to the
