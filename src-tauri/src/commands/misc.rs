...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/farion1231/ak-switch/releases/latest",
            None::<String>,
        )
...
///
/// 判定顺序（命中即返回）：
/// ① Hermes → `<bin_path 绝对> update`;Hermes CLI 自己知道安装环境,避免 ak-switch
...
echo "[ak-switch] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[ak-switch] Command exited. Press any key to close."
read -n 1 -s
"#,
...
mod tests {
    ...
    mod anchored_upgrade {
        ...
        fn hermes_uses_cli_update_anchor() {
            // Hermes 自带 `hermes update`;锚定到命令行默认那处 CLI,避免 ak-switch 猜
    ...
    mod install_strategy {
        ...
        fn hermes_install_uses_official_installer() {
            // Hermes 官方 installer 会处理 Python 3.11+/uv 等运行时;不要再从 ak-switch
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("ak-switch-missing-{unique}"));
