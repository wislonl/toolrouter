...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("akswitch://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.ak-switch/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/ak-switch.log）
            {
                ...
                let log_file_path = log_dir.join("ak-switch.log");
            ...
            let db_path = app_config_dir.join("ak-switch.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 ToolRouter 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.toolrouter.desktop/applications/ak-switch-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/ak-switch-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first akswitch:// URL
                        }
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::list_vscode_plugin_targets,
            commands::read_vscode_plugin_status,
            commands::sync_vscode_plugin_provider,
            commands::clear_vscode_plugin_provider,
            commands::preview_vscode_plugin_changes,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 akswitch://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older ToolRouter version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including ak-switch.db)\n\
            2) If you see “database version is newer”, please upgrade ToolRouter\n\
