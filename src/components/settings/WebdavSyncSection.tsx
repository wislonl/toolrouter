...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "ak-switch-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "ak-switch-sync",
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "ak-switch-sync";
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "ak-switch-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "ak-switch-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
